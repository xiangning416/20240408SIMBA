...
#the goal of this dataset file is to be able to take the offset file that yuening will send and then incorporate it into the positions file before everything gets written out. 
...
class MERFISHDataSet(ImageDataSet):
    ...
    def _import_positions_from_metadata(self):
        ...
        positionPath = os.sep.join([self.analysisPath, 'positions.csv']) #this means that the position file is just in the open folder so we can pull the offset file from there as well 
        #since positions.csv is just saved to the testdata folder, then self.analysisPath must just lead to the testdata folder 

        #load in offset data  
        df = pandas.read_csv(os.sep.join([self.dataHome, 'offsets.csv'])) #assumes that the offset data is stored in a csv file with this name in the datahome folder with the raw data 
        positionData= df.values

        #edit the positionData object that is written out, just fill it with the offsetdata, since that will be the actual positions

        #write the positionData to csv 
