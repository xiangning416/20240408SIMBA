...
class CellPoseSegment(FeatureSavingAnalysisTask):
    ...
    def __init__(self, dataSet, parameters=None, analysisName=None):
        ...
        if 'membrane_channel' not in self.parameters:
            self.parameters['membrane_channel'] == 'WGA'
...
class CellPoseSegmentSingleChannel(FeatureSavingAnalysisTask):
    ...
    def _run_analysis(self, fragmentIndex):
        ...
        # preprocess the images 
        #YL  seg_images_pp = self.preprocess_image_channels(seg_images)

        #Yl  if self.parameters['dump_preprocessed_images']:
            #YL  self._save_tiff_images(fragmentIndex, 'preprocessed_seg_images', seg_images_pp)

        # Combine the images into a stack
        zero_images = np.zeros(seg_images.shape)
        #Yl  stacked_images = np.stack((zero_images, zero_images, seg_images_pp), axis=3)
        stacked_images = np.stack((zero_images, zero_images, seg_images), axis=3)
        ...
        if self.parameters['path_to_user_model']:
            ...
            masks, flows, styles    = model.eval(stacked_images, 
                                            diameter=None, 
                                            do_3D=False, channels=[0, 0], 
                                            resample=True, min_size=self.parameters['min_size'])#Yl  do_3D=False, channels=[3, 0],
        else:
            # Load the cellpose model. 'cyto2' performs better than 'cyto'.
            # Use also for dapi 
