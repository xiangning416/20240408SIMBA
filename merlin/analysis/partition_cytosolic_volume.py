...
class PartitionBarcodesCytosolic(analysistask.AnalysisTask):
    ...
    def _run_analysis(self):
        ...
        touch_eroded = self.parameters['DAPI_touch_eroded_zarr']
        ...
        currentCells = sDB.read_features()
        cells = [cell._uniqueID for cell in currentCells]
        #volume calculations
        px_area = microns_per_pixel**2
        pixel_counts_per_z = {}
        for cell in cells:
            pixel_counts_per_z[cell] = {}
        ...
        for z_index in range(numz): 
            ...
            DAPI_masks = rescale(zarr.open(touch_eroded)[z_index,:,:], [scale_factor, scale_factor], order=0)
            
            binary_DAPI_masks = np.where(DAPI_masks != 0, 0, 1)
            cytosolic_masks =  masks * (binary_DAPI_masks == 1)

            #calculate volume of each cytosolic mask 
            flattened_array = cytosolic_masks.flatten()
            # Efficiently calculate the pixel counts for each unique value in flattened_array
            unique, counts = np.unique(flattened_array, return_counts=True)
            pixel_counts = dict(zip(unique, counts))
            # Now, calculate the area for each cell and store it, skipping the zero
            area_dict = {cell: pixel_counts.get(int(cell), 0) * px_area for cell in cells if int(cell) in pixel_counts}

            for cell, area in area_dict.items():
                if cell != 0:  # Exclude background or excluded areas
                    pixel_counts_per_z[cell][z_index*1.5] = area

            xmax = DAPI_masks.shape[1]
            ymax = DAPI_masks.shape[0]

            unique_values=np.unique(cytosolic_masks)
            ...
            for index, row in z_txs_scaled.iterrows():
                ...
                if True:
                    ...
                    if x >= 0 and y >= 0 and x < xmax and y < ymax: # check to see if this is the issue by seeing what actually gets compared 
                        #print("true1")
                        cell_index = cytosolic_masks[y, x] # indexing is flipped - first index is y in images
                        if cell_index:
                            ...
                            cell_id_list.append(int(cell_index))
            ...
            cxg_filename = f'cytosolic_cxg_z{z_name}'
        ...
        partitioned_transcripts = pandas.DataFrame({'cell_id': cell_id_list, 'barcode_id': barcode_id_list, 'global_x': global_x_list, 'global_y': global_y_list, 'z': z_list}).astype(float)
        self.dataSet.save_dataframe_to_csv(dataframe=partitioned_transcripts,resultName='cytosolic_partitioned_txs',analysisTask=self.get_analysis_name())
        
        cyto_vols = {}
        for key in pixel_counts_per_z.keys():
            zPos = list(pixel_counts_per_z[key].keys())
            zDiff = np.diff(zPos)
            zNum = np.array([[x, x + 1] for x in range(len(zPos) - 1)])
            areas = np.array(list(pixel_counts_per_z[key].values()))
            cyto_vol = np.sum([np.mean(areas[zNum[x]]) * zDiff[x] for x in range(zNum.shape[0])])
            cyto_vols[key] = cyto_vol
        
        for cell in currentCells:
            cellID = cell._uniqueID
            cell_txs = partitioned_transcripts[partitioned_transcripts["cell_id"] == cellID]
            cyto_vol = cyto_vols[cellID]
            cell.set_cyto_volume(cyto_vol)
            cell.set_transcript_list(cell_txs)

        self.get_feature_database().write_feature_transcripts(currentCells, 0) # write the transcripts for each cell to the feature database (hdf5 object)
        ...
        for z_index in range(numz):
            z_name=z_index+self.parameters['starting_z']
            file_name = f'cytosolic_cxg_z{z_name}'
        ...
        self.dataSet.save_dataframe_to_csv(dataframe=merged_df,resultName='cytosolic_cxg_combined',analysisTask=self.get_analysis_name())
