...
class PartitionBarcodes_DAPI(analysistask.AnalysisTask):
    ...
    def _run_analysis(self):
        ...
        # partitioned_transcripts = pandas.DataFrame(columns= ['cell_id', 'nuclear_id', 'barcode_id', 'global_x', 'global_y', 'z'])
        cell_id_list = []
        nuclei_id_list = []
        barcode_id_list = []
        global_x_list = []
        ...
        for z_index in range(numz): 
            ...
            masks = rescale(zarr.open(os.path.join(path_to_zarrs, zarr_name))[z_index,:,:], [1, 1], order=0) #load in the zarr file of the masks file for the given z 
            #contain pixel wise labelling of which pixel is a cell, which cell it is, and which ones are not 
            DAPI_masks = rescale(np.load(DAPI_npy)[z_index,:,:], [s_scaling_factor, s_scaling_factor], order=0)
                ...
                if True:
                    ...
                    if x >= 0 and y >= 0 and x < xmax and y < ymax: # check to see if this is the issue by seeing what actually gets compared 
                        ...
                        if cell_index:
                            ...
                            global_y_list.append(float((row['global_y']+minx)*microns_per_pixel))
        ...
        partitioned_transcripts = pandas.DataFrame({'cell_id': cell_id_list, 'nuclear_id': nuclei_id_list, 'barcode_id': barcode_id_list, 'global_x': global_x_list, 'global_y': global_y_list, 'z': z_list}).astype(float)
