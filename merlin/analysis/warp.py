...
class FiducialCorrelationWarp(Warp):
    ...
    def bella_filter(self, inputImage) -> np.ndarray:

        contrast = 10. # Contrast control ( 0 to 127)
        brightness = 1. # Brightness control (0-100)

        out = cv2.addWeighted(inputImage, contrast, inputImage, 0, brightness)
    ...
    def _run_analysis(self, fragmentIndex: int):
        ...
        fixedImage = self.bella_filter(
            self.dataSet.get_fiducial_image(0, fragmentIndex))
            
            
            
        # with self.dataSet.writer_for_analysis_images(
        #             self, 'fixed_images', fragmentIndex) as outputTif:
        #     outputTif.save(
        #                     fixedImage,
        #                     photometric='MINISBLACK',
        #                     contiguous=True)
        ...
        # imgs = [self.bella_filter(
        #         self.dataSet.get_fiducial_image(x, fragmentIndex)) for x in
        #            self.dataSet.get_data_organization().get_data_channels()] 
        # with self.dataSet.writer_for_analysis_images(
        #                     self, 'fixed_images_imground', fragmentIndex) as outputTif:
        #     for im in imgs:
        #             outputTif.save(
        #                             im,
        #                             photometric='MINISBLACK',
