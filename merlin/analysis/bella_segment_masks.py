    ...
    def _run_analysis(self) -> None:
        ...
        for z in range(len(allz_boundaries)):
            ...
            for bound in allz_boundaries[z]:
                poly_centroid = Polygon(bound).centroid
                x,y = int(poly_centroid.x), int(poly_centroid.y) #in pixel coordinates
                #TODO check this!
                try:
                    # bound_dict[mask_matrix[z][y][x]][z+self.parameters['starting_z']] != None: #if cell already exists in dictionary
                    bound_dict[mask_matrix[z][y][x]][(z+self.parameters['starting_z'])*1.5].append(Polygon(micron_allz_boundaries[z][counter])) #add new geometry to list (allows for multiple geometries per cell) -- in micron coordinates
                    # print("multiple geometries")
                except:
                    try:
                        bound_dict[mask_matrix[z][y][x]][(z+self.parameters['starting_z'])*1.5] = [Polygon(micron_allz_boundaries[z][counter])] #TODO add z & use list to allow for multiple geometries per cell
                    except:
                        bound_dict[mask_matrix[z][y][x]] = {} #TODO add z & use list to allow for multiple geometries per cell
                        bound_dict[mask_matrix[z][y][x]][(z+self.parameters['starting_z'])*1.5] = [Polygon(micron_allz_boundaries[z][counter])]
                counter +=1
        ...
        for cell_id in bound_dict.keys():
            ...
            for z in bound_dict[cell_id].keys():
                ...
                # print(len(bound_dict[cell_id][z]))
                # print(z)
                z_levels.append(z)
