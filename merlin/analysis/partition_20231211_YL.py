        ...
        for z_index in range(numz): 
            ...
            for index, row in txs_scaled.iterrows():
                ...
                if z == z_index+self.parameters['starting_z']:
                    x = math.ceil(row['global_x_shifted']) if row['global_x_shifted'] % 1 >= 0.444445 else math.floor(row['global_x_shifted'])
                    y =  math.ceil(row['global_y_shifted']) if row['global_y_shifted'] % 1 >= 0.444445 else math.floor(row['global_y_shifted'])
